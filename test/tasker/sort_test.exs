defmodule Tasker.SortTest do
  ...
  defp assert_correct_order(tasks, expected_order) do
    assert Tasker.Operations.sort(tasks) == Enum.map(expected_order, &Map.delete(&1, "requires"))
  end
  ...
  test "one task with unknown require" do
    ...
    assert Tasker.Operations.sort([task]) == {:error, "Invalid requires in task test1"}
  end
  ...
  test "two tasks one with unknown require" do
    ...
    assert Tasker.Operations.sort([task1, task2]) == {:error, "Invalid requires in task test2"}
  end
